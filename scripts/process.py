...
def increment_month(latest_date_str):
    latest_date_dt = datetime.strptime(latest_date_str.strip(), "%Y-%m")
...
def process_final_data():
    # Make it monthly data: combine daily observations into monthly averages.
...
def combine_and_remove():
    # Combine us-10y.csv and us-10y-final.csv, deduplicate by month (last value wins),
    # then write back to us-10y.csv.
    with open("data/us-10y.csv", 'r') as file:
        csv_reader = csv.reader(file)
        header = next(csv_reader)
        existing = list(csv_reader)
    ...
    # Merge: build ordered dict keyed by month; new rows overwrite existing
    merged = {}
    for row in existing + new_rows:
        if row:
            merged[row[0].strip()] = row[1].strip()

    # Write back with LF line endings
    with open("data/us-10y.csv", 'w', newline='') as file:
        csv_writer = csv.writer(file, lineterminator='\n')
        csv_writer.writerow(header)
        for month, value in merged.items():
